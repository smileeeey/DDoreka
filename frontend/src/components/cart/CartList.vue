  ...
  <table class="simple_table">
    ...
    <tbody v-if="items">
      <tr v-for="(item, idx) in items" :key="idx">
        ...
        <td>
          <v-img :src="`data:image/jpeg;base64,${item.img}`" height="120" width="120"> </v-img>
        </td>
        <td>
          <div style="display: flex; justify-content: space-between;">
            ...
            <div style="display: flex;">
              ...
              <select class="quantity-select" style="margin-left: 1rem; margin-right: 3rem;" v-model="items[idx].amount">
      ...
      <tr class="total" style="">
        <td colspan="5" style="border-top: 2px solid #969696; text-align: right !important; font-size: 16px;">
          상품가격 <strong>{{ totalCost | comma }}</strong
          >원 + 배송비 <strong>무료</strong> = 주문금액 <strong style="color: red;">{{ totalCost | comma }}</strong
          >원
        </td>
      </tr>
    </tbody>
...
export default {
  ...
  methods: {
    ...
    deleteCartitem(idx) {
      ...
      user
        .delete(
          `/user/cart/${this.items[idx].cartId}`,
          {},
          {
            headers: {
