<template>
  <v-slide-item @click.native="moveItemDetail">
    <v-card class="ma-4" height="200" width="200">
      ...
      <v-img :src="`data:image/jpeg;base64,${steadySeller.thumbnail}`" height="150" width="200"></v-img>
      <v-card-text class="pa-1 ml-2">
        ...
        <v-row align="center" justify="start" class="mx-0">
          <!-- 평균평점 -->
          <v-rating :value="steadySeller.rating" color="warning" background-color="warning lighten-1" dense half-increments readonly size="16"></v-rating>

          <!-- 평균평점, 리뷰개수 -->
          <div class="ml-1">({{ steadySeller.review_cnt }})</div>
        </v-row>
      </v-card-text>
    </v-card>
  </v-slide-item>
