<template>
  <v-card :to="{ name: 'ItemDetail', params: { id: item.category3Id, productid: productId } }">
    <v-img :src="sumnailUrl" aspect-ratio="1" class="grey lighten-2">
    ...
    <v-card-text>
      ...
      <v-row align="center" class="mx-0 py-1">
        <v-rating :value="item.rating" color="warning" background-color="warning lighten-1" dense half-increments readonly size="17"></v-rating>

        <div class="grey--text ml-1">({{ item.review_cnt }})</div>
      </v-row>
    </v-card-text>
  </v-card>
</template>
        ...
        axios
          ...
          .then((res) => {
            ...
            if (this.sFiles.length > 0) {
              // this.sumnailUrl = `file:k4d104.p.ssafy.io/home/upload/image/${this.sFiles[0].path}/${this.sFiles[0].systemName}`
