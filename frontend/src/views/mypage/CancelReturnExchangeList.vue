...
export default {
  ...
  created: function () {
    let dataArray = [];
    axios.get(`http://k4d104.p.ssafy.io:8084/order/userid/${this.userId}/status/3`)
      .then(res => {
        ...
        for (let j=0; j<array.length; j++) {
          ...
          axios.get(`http://k4d104.p.ssafy.io:8081/product/detail/${array[j].productId}`)
            .then(resp => {
              ...
              axios.get(`http://k4d104.p.ssafy.io:8082/file/fileServe/${resp.data.data.images[0].fileId}`)
                .then(respo => {
                  dataArray.push({
