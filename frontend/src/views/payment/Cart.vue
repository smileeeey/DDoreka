      ...
      <v-row>
        <v-col cols="12">
          <div class="box">
            ...
            <div v-show="e1 == 1">
              <CartList :items="items" style="width: 100%;" @updateTotalCost="updateTotalCost" />
              <div style="display: flex; justify-content: center; margin-top: 3rem;">
                <v-btn
                  style="background-color: white; color: #0275d8; width: 200px; height: 50px;
                ...
                <v-btn
                  style="background-color: #0275d8; color: white; width: 200px; height: 50px;
...
export default {
  ...
  computed: {
    ...mapState(['wishlist', 'email', 'name', 'phone', 'userId']),
  },
  watch: {
    e1: function() {
          ...
          if (this.items[i].select == true) {
            ...
            order
              ...
              .then((res) => {
                ...
                user.delete(`/user/cart/${this.items[i].cartId}`).then((r) => {
                  console.log(r);
  ...
  created: function() {
    ...
    for (let i = 0; i < this.wishlist.length; i++) {
      ...
      product.get(`/product/detail/${this.wishlist[i].productId}`).then((detailres) => {
        ...
        if (detailres.data.data.images.length > 0) {
          file.get(`/file/fileServe/${detailres.data.data.images[0].fileId}`).then((fileres) => {
            tmpitems.push({
              sellerId: sellerId,
              cartId: cartId,
