      ...
      if (this.e1 == 3) {
        ...
        for (let i=0; i<this.items.length; i++) {
          if (this.items[i].select == true) {
            console.log('order')
            axios.post('http://k4d104.p.ssafy.io:8084/order', {
              'userId': this.userId,
              ...
              .then(res => {
                ...
                axios.delete(`http://k4d104.p.ssafy.io:8085/user/cart/${this.items[i].cartId}`)
                  .then(r => {
                    console.log(r)
      ...
      axios.get(`http://k4d104.p.ssafy.io:8081/product/detail/${this.wishlist[i].productId}`)
        .then(detailres => {
          ...
          if (detailres.data.data.images.length > 0) {
            axios.get(`http://k4d104.p.ssafy.io:8082/file/fileServe/${detailres.data.data.images[0].fileId}`)
              .then(fileres => {
                tmpitems.push({
                  sellerId: sellerId,
                  cartId: cartId,
