      ...
      <v-container fluid style="width: 500px;">
        <v-row>
          <v-col cols="12" style="padding-bottom: 0;">
            <v-img @click="$router.push({ name: 'Main' })" style="cursor: pointer;" src="@/assets/eureka_logo(blank).png" class="my-auto" max-height="150" max-width="500"> </v-img>
          ...
          <v-col cols="12">
            <v-btn
              style="background-color: white; color: #0275d8; 
            font-size: 1.5rem; font-weight: bold; 
            width: 100%; height: 150%;"
...
export default {
  ...
  methods: {
    ...
    login: function() {
      this.SET_LOGOUT();
      axios
        .post('http://k4d104.p.ssafy.io:8088/login', {
        ...
        .then((res) => {
          ...
          axios.get(`http://k4d104.p.ssafy.io:8088/seller/getByEmail/${this.form.email}`, {}, {}).then((response) => {
            console.log('2nd');
            console.log(response.data);
            this.$store.dispatch('SETSELLERINFO', response.data);
            console.log(response.data);
            axios
              .get(`http://k4d104.p.ssafy.io:8088/store/getBySellerId/${response.data.id}`)
              .then((res) => {
                console.log(res.data);
                console.log('판매자 로그인 데이터');
