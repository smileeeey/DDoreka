...
export default {
  ...
  created: function () {
    console.log("created")
    axios.get(`http://k4d104.p.ssafy.io:8081/product/seller/all/${this.seller.id}`)
      .then(res => {
          ...
          axios.get(`http://k4d104.p.ssafy.io:8084/order/prodcut/${productslist[i].id}/dayofweek`)
            .then(r => {
              ...
              if (productslist[i].images.length > 0) {
                axios.get(`http://k4d104.p.ssafy.io:8082/file/fileServe/${productslist[i].images[0].fileId}`)
                  .then(res => {
