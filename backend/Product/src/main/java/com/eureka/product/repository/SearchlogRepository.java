...
public interface SearchlogRepository extends JpaRepository<Searchlog, Integer> {
    ...
    @Query(value= "select search_word from searchlog where category1id= ?1 and search_date > DATE_ADD(now(),INTERVAL -1 hour) group by category1id,search_word order by count(*) DESC limit 0,10",nativeQuery = true)
